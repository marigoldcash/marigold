...
Phase 5 of [PLAN.md](../../PLAN.md). A complete written spec of the note pool, frozen and externally reviewed (P5.9) before any implementation begins (Phase 6+). Each `## P5.N` section below is that plan step's deliverable — do not implement against this document until P5.9 has closed.
...
7. **P5.8**: the 3-of-5 trust boundary stated prominently up front; the IBD/bootstrap trust root made explicit (software distribution as root, an anchor-ratchet persistence rule, multi-peer anchor query, suppression → staleness alerting, residual eclipse risk named); equivocation evidence lifecycle specified end to end; the T/M/K quantitative sensitivity model made a hard pre-launch gate (flagged into PLAN's P9.5).
...
- **Handover bundle** (added 2026-09-15, PLAN P8.0f; text prefix `marigoldpay:`): a payment handed over as one code. The payer's wallet makes **one fresh key** for the handover and, in one `TransferOp`, produces the amount's notes *and one 0.01 stamp* under that key (change back to the payer's own fresh keys). Payload: `sk: [u8; 32] || N × (sn: [u8; 32] || d: u8)` — 32 + 33 N bytes, against 65 N for N single-note bearer payloads, and the receiver's import is one `SignedGroup` signature for all N. The receiver's wallet verifies every serial on chain under `pk(sk)`, stores the key, and immediately rotates every note but the stamp to fresh keys only it holds, the rotation paid with the stamp — so a receiver who holds nothing can still end up with money only they can spend, and the sender has paid for it. Bearer, like cash: whoever holds the code holds the money until that rotation lands. The single-note bearer payload (`marigoldnote:`, 65 bytes) stays for moving keys between wallets one person controls, with no rotation on import.
...
## P5.9 — Time-locked notes (v1.2, PLAN P8.0g)
