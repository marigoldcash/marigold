...
STATE.md already lists handle registration as an open item — it becomes urgent the moment the repo is public, because squatters move on announced projects. Register within days: **X/Twitter** (`marigoldcash` + defensive `marigoldcoin`), **Discord server** (this becomes the support hub — miners expect it; channels: #announcements, #testnet, #mining-support, #dev), **Telegram** (announcement mirror), **Reddit** (claim r/marigoldcash; post there even if quiet), **YouTube, Docker Hub, npm** (defensive). GitHub org exists. Recommendation: Discord is the one that needs actual daily attention; everything else can be low-volume mirrors of announcements at first.
