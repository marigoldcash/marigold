...
/// Address prefix identifying the network type this address belongs to (such as `marigold`, `marigoldtest`, `marigoldsim`, `marigolddev`).
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
#[borsh(use_discriminant = true)]
pub enum Prefix {
    #[serde(rename = "marigold")]
    Mainnet,
    #[serde(rename = "marigoldtest")]
    Testnet,
    #[serde(rename = "marigoldsim")]
    Simnet,
    #[serde(rename = "marigolddev")]
    Devnet,
    #[cfg(test)]
    A,
    #[cfg(test)]
...
impl Prefix {
    fn as_str(&self) -> &'static str {
        match self {
            Prefix::Mainnet => "marigold",
            Prefix::Testnet => "marigoldtest",
            Prefix::Simnet => "marigoldsim",
            Prefix::Devnet => "marigolddev",
            #[cfg(test)]
            Prefix::A => "a",
            #[cfg(test)]
            Prefix::B => "b",
...
impl TryFrom<&str> for Prefix {
    ...
    fn try_from(prefix: &str) -> Result<Self, Self::Error> {
        match prefix {
            "marigold" => Ok(Prefix::Mainnet),
            "marigoldtest" => Ok(Prefix::Testnet),
            "marigoldsim" => Ok(Prefix::Simnet),
            "marigolddev" => Ok(Prefix::Devnet),
            #[cfg(test)]
            "a" => Ok(Prefix::A),
            #[cfg(test)]
            "b" => Ok(Prefix::B),
...
/// Kaspa [`Address`] struct that serializes to and from an address format string: `marigold:qz0s...t8cv`.
///
/// @category Address
...
mod tests {
    ...
    fn cases() -> Vec<(Address, &'static str)> {
        ...
        vec![
            ...
            (Address::new(Prefix::Testnet, Version::PubKey, &[0u8; 32]),      "marigoldtest:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqmmc0v0cg"),
            (Address::new(Prefix::Testnet, Version::PubKeyECDSA, &[0u8; 33]), "marigoldtest:qyqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqmpza349a"),
            (Address::new(Prefix::Testnet, Version::PubKeyECDSA, b"\xba\x01\xfc\x5f\x4e\x9d\x98\x79\x59\x9c\x69\xa3\xda\xfd\xb8\x35\xa7\x25\x5e\x5f\x2e\x93\x4e\x93\x22\xec\xd3\xaf\x19\x0a\xb0\xf6\x0e"), "marigoldtest:qxaqrlzlf6wes72en3568khahq66wf27tuhfxn5nytkd8tcep2c0vrs4zdp5xeq"),
            (Address::new(Prefix::Mainnet, Version::PubKey, &[0u8; 32]),      "marigold:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqql2k57z6w"),
            (Address::new(Prefix::Mainnet, Version::PubKey, b"\x5f\xff\x3c\x4d\xa1\x8f\x45\xad\xcd\xd4\x99\xe4\x46\x11\xe9\xff\xf1\x48\xba\x69\xdb\x3c\x4e\xa2\xdd\xd9\x55\xfc\x46\xa5\x95\x22"), "marigold:qp0l70zd5x85ttwd6jv7g3s3a8llzj96d8dncn4zmhv4tlzx5k2jyfmdxkcx9"),
        ]
        // cspell:enable
    }
    ...
    fn test_errors() {
        // cspell:disable
        let address_str: String = "marigold:qqqqqqqqqqqqq1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqql2k57z6w".to_string();
        ...
        let address_str: String = format!("marigold:qqqqqqqqqqqqq{invalid_char}qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqql2k57z6w");
        let address: Result<Address, AddressError> = address_str.try_into();
        ...
        let address_str: String = "marigold1:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqql2k57z6w".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::InvalidPrefix("marigold1".into())), address);

        let address_str: String = "marigoldqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqql2k57z6w".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::MissingPrefix), address);

        let address_str: String = "marigold:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqql2k57z6x".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);

        let address_str: String = "marigold:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqql2k57z6w".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);
        // cspell:enable
    }
