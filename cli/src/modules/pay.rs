...
impl Pay {
    ...
    async fn pay_locked(ctx: &Arc<KaspaCli>, amount: &str, key: &str, rest: &[String]) -> Result<()> {
        ...
        ctx.record("offered", result.value_petals, result.stamp_petals + result.transfer.fee_petals, format!("locked {}", crate::cli::humanised_minutes(seconds / 60)), result.transfer.transaction_id.to_string());
        tprintln!(ctx, "");
