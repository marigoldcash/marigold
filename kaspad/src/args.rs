...
0, Tracker::MAX_ADDRESS_UPPER_BOUND, Tracker::DEFAULT_MAX_ADDRESSES)),
        ...
        .arg(
            Arg::new("ua_rule")
                ...
                .action(ArgAction::Append)
                .require_equals(true)
                .help("User agent admission rule. Forms: allow;regex:<regex>, reject;regex:<regex>, allow;ver:<name><op><version>, reject;ver:<name><op><version>. Version operators: <, <=, >, >=, ==. Example: --ua-rule='reject;ver:marigoldd<1.1.1'. Policy: if allow rules exist and none match, reject; if any reject rule matches, reject; otherwise accept."),
        )
        .arg(
            Arg::new("externalip")
                .long("externalip")
