...
- The **serial** uniquely identifies the note for its whole life. It is derived, not assigned: `serial = H(creating_txid || output_index)`, a domain-separated hash — the same outpoint pattern the host chain already uses for UTXOs, requiring no counter and no registry of "next serial."
...
| **Rotate** (transfer) | a note's current key's signature | the same note under a fresh key | no |
...
Pool-operation authorization is a BIP340 Schnorr signature by the note's current key over a domain-separated hash that binds the operation, its consumed serials, the enclosing transaction's ledger outputs (preventing redirect malleability), and a **freshness anchor** — a recent DAA score after which the signature expires (about one hour). Expiry bounds the shelf life of any signed-but-unbroadcast operation: a photographed payment QR or a stale invoice stops being a live liability within the session that created it, closing the class of replay problems bearer instruments are historically prone to. Replay of *executed* operations is impossible by construction, since a confirmed rotation changes the very key a replayed signature would need to verify against.
...
This section makes the claims exact, because a cash-like system must be honest about its envelope. **Everything on the chain is public and unencrypted.** For every note, at every moment: its serial, its denomination, its current public key, and its complete operational history — when it was minted, every rotation, every split and merge it participated in, when it was redeemed. Amounts are never confidential. Total pool composition per denomination is public. Mint and redeem visibly connect ledger funds to specific serials at entry and exit.
...
- **The operation graph is visible.** Serials persist across rotations, so a note's timeline is one public thread; splits and merges connect threads. Timing correlations, denomination patterns, and fee-stamp lineage are all analyzable graph structure.
...
- **Pruning is a storage optimization, not an eraser.** Anyone running an archival node retains the complete operation history; the design assumes at least one always exists.
